...
private extension GIFSource {
    func loadData(session: URLSession, cache: URLCache, fileManager: FileManager) async throws -> Data {
        ...
        case .remote(let url):
            ...
            if url.isFileURL {
                return try await url.path.loadData(fileManager: fileManager)
            }
            return try await url.loadData(session: session, cache: cache)
        case .local(let filePath):
            guard let filePath = filePath else {
                throw URLError(.fileDoesNotExist)
            }
            return try await filePath.loadData(fileManager: fileManager)
        }
    }
}
